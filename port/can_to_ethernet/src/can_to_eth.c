...
int32_t can_to_eth(ETH_MODE mode)
{
    ...
    {
        case ETH_TCP_SERVER:
            process_tcp_server(SOCKET_TCP, p_dev_config->connection_config.local_port);
            break;
        case ETH_TCP_CLIENT:
            process_tcp_client(SOCKET_TCP, p_dev_config->connection_config.remote_ip, p_dev_config->connection_config.remote_port);
            break;
        default:
            break;  
    }
